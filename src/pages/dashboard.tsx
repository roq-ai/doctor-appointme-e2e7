...
import { useRoqPlatformUserProfiles, useInsuranceCount, useBillingCount, useMedicalRecordCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const insuranceStat = useInsuranceCount();

  const billingStat = useBillingCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Insurances',
      result: insuranceStat?.data,
      isLoading: insuranceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Billings',
      result: billingStat?.data,
      isLoading: billingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
