...
import { useRoqClient, useAppointmentFindManyWithCount } from 'lib/roq';
...
export function AppointmentListPage(props: AppointmentListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: [
            'user_appointment_patient_idTouser',
            'user_appointment_doctor_idTouser',
            'clinic',
            'billing.count',
          ],
  ...
  const { data, error, isLoading, mutate } = useAppointmentFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.appointment.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.appointment_date ? format(record?.appointment_date, 'dd-MM-yyyy') : '',
    },
    {
      id: 'appointment_time',
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.appointment_time ? format(record?.appointment_time, 'dd-MM-yyyy') : '',
    },
    { id: 'status', header: 'Status', accessorKey: 'status' },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_appointment_patient_idTouser',
          header: 'User Appointment Patient Id Touser',
          accessorKey: 'user_appointment_patient_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_appointment_patient_idTouser?.id}`}
            >
              {record.user_appointment_patient_idTouser?.email}
            </Link>
          ),
        }
      : null,
    ...
    hasAccess('clinic', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'clinic',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/clinics/view/${record.clinic?.id}`}>
              {record.clinic?.name}
            </Link>
          ),
        }
      : null,
    hasAccess('billing', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'billing',
          header: 'Billing',
          accessorKey: 'billing',
          cell: ({ row: { original: record } }: any) => record?._count?.billing,
        }
      : null,
