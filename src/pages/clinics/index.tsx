...
import { useRoqClient, useClinicFindManyWithCount } from 'lib/roq';
...
export function ClinicListPage(props: ClinicListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'appointment.count', 'billing.count', 'insurance.count'],
  ...
  const { data, error, isLoading, mutate } = useClinicFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.clinic.delete({ where: { id } });
      await mutate();
    } catch (error) {
