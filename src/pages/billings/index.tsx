...
import { useRoqClient, useBillingFindManyWithCount } from 'lib/roq';
...
export function BillingListPage(props: BillingListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'clinic', 'appointment'],
  ...
  const { data, error, isLoading, mutate } = useBillingFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.billing.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'payment_status', header: 'Payment Status', accessorKey: 'payment_status' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.billing_date ? format(record?.billing_date, 'dd-MM-yyyy') : '',
    },
